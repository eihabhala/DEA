...
//--- Include only standard MT5 libraries
#include <Trade/Trade.mqh>
#include <Trade/PositionInfo.mqh>
#include <Trade/OrderInfo.mqh>

//--- Input parameters - SIMPLIFIED MT5 VERSION
...
int OnInit()
{
    Print("=== AI Trading Expert MT5 Simple Version Initialization ===");
...
{
    Print("AI Trading Expert MT5 deinitialized, reason: ", reason);
...
void OnTick()
{
    ...
    if(iTime(g_CurrentPair, PERIOD_CURRENT, 0) != last_bar_time)
    {
        last_bar_time = iTime(g_CurrentPair, PERIOD_CURRENT, 0);
        Print("New bar - MT5 EA is working with all parameters accessible");
    }
}
...
void UpdateMarketData()
{
    ...
    {
        g_CurrentSpread = (int)((tick.ask - tick.bid) / SymbolInfoDouble(g_CurrentPair, SYMBOL_POINT));
    }
...
{
    //--- Calculate ATR using MT5 functions
    int atr_handle = iATR(g_CurrentPair, PERIOD_CURRENT, InpATRPeriod);
    ...
    //--- Calculate channel lines using MT5 functions
    int highest_handle = iHighest(g_CurrentPair, PERIOD_CURRENT, MODE_HIGH, 20, 1);
    int lowest_handle = iLowest(g_CurrentPair, PERIOD_CURRENT, MODE_LOW, 20, 1);
    
    double high_price = iHigh(g_CurrentPair, PERIOD_CURRENT, highest_handle);
    double low_price = iLow(g_CurrentPair, PERIOD_CURRENT, lowest_handle);
    
    g_ATRMiddleLine = (high_price + low_price) / 2.0;
...
bool CreateDashboard()
{
    ...
    //--- Title
    CreateLabel("AI_Title", "🤖 AI Trading Expert MT5 Dashboard", 30, 40, clrYellow);
